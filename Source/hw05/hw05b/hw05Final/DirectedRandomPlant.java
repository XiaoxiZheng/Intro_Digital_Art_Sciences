...
class ImageFrame extends JFrame{
	...
	protected void CreateBufferedImageT(){
		...
		image.drawStems(startColor,endColor,stems,stepsPerStem,alpha,deltaTheta,deltaRho);
		...
		new Thread(new Runnable() {
						...
						public void run() {
							...
							for (int i = 0; i < stepsPerStem; i++) {
								...
								// Test sleeping to see the actual process
/*								try {
									if (i % 3 == 0)
										Thread.sleep(10000);
...
class PlantImage extends BufferedImage {
		...
		protected void drawStems(int startColor_, int endColor_,int stems, int stepsPerStem, double alpha, double deltaTheta, double deltaRho) {
			...
			for (int j = 0; j < stems; j++) {
				stemObj[j] = new stem(colorArray, basicStrokeWeight,stepsPerStem,alpha,deltaTheta,deltaRho);
		...
		protected class stem{
			...
			public stem(Color[] color_, BasicStroke[] strokes_,int stepsPerStem_, double alpha_, double deltaTheta_, double deltaRho_){
